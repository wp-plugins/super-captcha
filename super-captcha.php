...
class newSuperCaptcha
	...
	var $db_table_name = '';
	var $scVersion = '3.0.1'; // Used to alert admins to change configs and offers version number inside contextual areas.
		...
		<div class="wrap">
		  ...
		  <div style="width:100%;overflow: hidden;height:auto;">
			...
			<h4>Is It Working?</h4>
			  ...
			  <?php else: ?>
			  <strong>In order to have this plugin listed at wordpress.org/plugins we had to remove the iFrame here to ensure the plugin is working before enabling.  In order to enable this iFrame, tick "enable testing" option and save.</strong>
			  <?php endif; ?>
				...
				<form method="post" action="options.php">
					...
					<table class="form-table">
						...
						<tr valign="top">
							...
							<td>
								<?php if ( $this->pro_license_check( ) == true ) : ?>
								<input style="width:300px;border-color:#009900!important;" type="text" name="pro_license" value="<?php echo esc_attr( get_option('pro_license') ); ?>" /> <input type="submit" style="color:#009900!important;" name="Revalidate &raquo;" class="button button-secondary" value="Revalidate &raquo;" />
								...
								<br />
								<span style="color:#ff0000;"><strong>Oh No! Invalid Key Entered.</strong></span>
								<?php else : ?>
						...
						<tr valign="top">
							...
							<td>
								<input type="checkbox" name="enable_test" value="true"<?php if( esc_attr( get_option('enable_test') ) == true ): ?> CHECKED<?php endif; ?> /> Allow Testing<br />
							</td>
						</tr>
	...
	function getCaptchaImage()
		{
		?>
		<iframe src="http://spam.goldsborowebdevelopment.com/verify/display.php?uid=<?php echo(urlencode(site_url())); ?>&sid=<?php echo(session_id()); ?>&f=<?php echo(esc_attr( get_option('font') )); ?>&bg=<?php echo(esc_attr( get_option('background') )); ?>&dis=<?php echo(esc_attr( get_option('distortion') )); ?>" style="border:none;width:262px;height:150px;">
	...
	function signup_bpform()
		...
		?>
		<div class="editfield">
				<label for="Image">Anti Bot Image</label>
		<?php
	...
	function signup_bppost()
		...
		if ( $this->validateCode( $_POST['SpamCode'] ) == true )
			{
			$bp->signup->errors['scaptcha'] = NULL;
			} else {
			$bp->signup->errors['scaptcha'] = ('Please enter correct verification code.');
			}
		}
	...
	function login_post( )
		...
		if ( $this->validateCode( $_POST['SpamCode'] ) != true )
			{
			$error = 'Please enter correct verification.';
			}
		}
	function comments_submit( $content )
		...
		if ( $this->validateCode( $_POST['SpamCode'] ) != true )
			{
			wp_die( __('<strong>ERROR</strong>: Please enter correct verification. <a href="' . $_SERVER['HTTP_REFERER'] . '">Try again</a>') );
			} else {
			return $content;
			}
		}
	function main_page_help()
		...
		$screen = get_current_screen();
		  ...
		  far as stopping automated blog creation and user signups.</p>
		  ...
		  a link-back in the footer of their site.  This both let bots know not to mess with thier site and gave us a little credit for making
		  the software.</p>
		  
		  <p>We\'ve become compliant by now hosting Super Captcha\'s image generator on our own servers with a link on the actual image. So
		  to bypass this, the link is being hosted by our own page, which is iframed from your site.  While this will cause a severe page-rank
		  loss, our preference was to put a simple "nofollow" link at the bottom of the websites using this software.</p>
		...
		$whatsnew = ('
			...
			<h3>Updates &amp; Fixes</h3>
			<ul>
			  <li>FEATURE: Widgets added in the Dashboard for easy IP reporting and IP checking.</li>
			  <li>FEATURE: One-Click ISP Abuse Reporting -- No research necessary.</li>
			  <li>Fix: Some spelling errors were corrected</li>
			</ul>
			
			<h3>Known Issues <small>on the list to be fixed</small></h3>
			<ul>
			  <li>Some fonts will not render properly.</li>
			  <li>Report to ISP button does not support IPv6</li>
			</ul>
			...
			<ul>
			  ...
			  <li>More intergration with other plugins such as BBPress and Askmet</li>
			  <li>Adjustable line color</li>
			  <li>Customizable Error Messages and Fields <small>likely to be in next release</small></li>
			</ul>
			');
...
');
	...
	function first_setup()
		{
		if( !empty( $_REQUEST['page'] ) && $_REQUEST['page'] != 'new-super-captcha/new-super-captcha.php') :
			?>
			<div class="updated"><p><a href="admin.php?page=new-super-captcha/new-super-captcha.php" style="float:right" class="button button-secondary">Take Me There &raquo;</a><strong>HAIL CAPTAIN!</strong><br />
