...
Tags: CAPTCHA, spam, anti-spam, super-captcha, bot, anti-bot, security, protection, buddypress, bbpress, 3D, admin, multisite, login, registration, comments, dashboard
...
Entire Security Service Suite centred around an easy 3D Captcha that provokes deduction skills to solve it.

== Description ==

You've been there, you have installed 20 or more CAPTCHA plugins for WordPress to see them ultimately fail in even slowing down spam bots. We know, we have been there too. This plugin
solves ALL of it. You see, this software is as simple as it gets. While everyone is trying to complicate their CAPTCHAs with images, flash puzzles, and coloring it like a 2 year old with
eight-inch markers, we decided to make an image provoke deduction skills. You see, a computer has no ability to reason, therefore it cannot use any power of deduction. What this means
is, if we lay lines over an invisible word, you can see what's missing to near-instantly know what the word is. But a computer that follows "edges" to find patterns, figuring out this
CAPTCHA becomes an impossibility.
...
The service provided by GoldsboroWebDevelopment is free and no API key or code is ever required.  At any time you can upgrade to Pro, in which you will be issued a license key.  The price
we charge for Pro is just enough to cover our basic costs, $1.99/mo.  Your price does not increase due to volume.

We didn't aim to just band-aid the spam issue, we aspire to permanently fix it.  To know what I mean, you really have to download the plugin and see, but a main feature that shows just what
our intentions are is when you do an IP lookup or submit an IP to us, we automatically query the abuse email for that IP and pre-populate an Abuse Report email so all you have to do is hit 
"Send". That's right, we're streamlining the abuse reporting process!
...
Because this feature puts a stone-wall between your site and bots, you can use it on nearly any form -- including your login form which will completely negate brute-force attacks on your site. A
brute-force attack is when a hacker or bot tries to guess your password by running a series of automated passwords through your site or by sequentially trying a new password such as 00000001 00000002
and so on.

We are also integrating some browser integrity checking using JavaScript to ensure that real web browsers are attempting to access your forms.  It is quite simple in the way it works.  JavaScript
simply changes a form field and if the submitted field doesn't check out on the server when it is submitted, we assume it was a bot.  The issue is if a user has JavaScript disabled, submitting forms
on your site will be an impossibility which is why this is not implemented yet.  Once we figure out how to implement this in a manner that any user can use it, we will.

Super Captcha was well-know for is customization.  You could sit for an hour tweaking how you wanted it to look, so we ensured this feature was a must-have in this new version.  You can change your
backgrounds, fonts, and eventually we'll even let you change you line color and change your solid backgrounds to any color you want.  Of course, we need a lot more time or a lot more funding for that.

This plugin was originally developed for BuddyPress and Multi-site in order to stop spam registrations and spam blogs.  But we had so many requests to put the form in the comments, we did just that. But
just keep in mind that this was really intended for Buddypress or Multi-Sites, not a comment spam buster (use Askmet for that!)

Have you ever seen a website where someone was selling OCR services?  You know, services that people can pay for which will allow them to break CAPTCHAs.  We ran across one once, and we found, they were
using our CAPTCHA.  If you want to know what locks you need for your house, inspect the locks on a cat-burler's house.  The same applies to CAPTCHAs, you can find ones that work by simply inspecting the
OCR sites and what systems they have in place.

To-date our Captcha's have had 99.87% success ratings, and Super Captcha Pro has seen a near 100% (99.99881%) over the past two years.  We hope you download this plugin and help us improve those
statistics by allowing this software to automatically report back to us what bots are up to out there!
...
Please read our forums on WordPress.org for intergration development, hooks, and more.
...
* :: -FEATURE:: Removed ability to test the plugin in your admin panel.  Sorry, this was demanded by WordPress.org (Askmet has this feature though)
* :: -FEATURE:: Removed ability to request a accidentally submitted IP to be removed. Again this was demanded by the WordPress.org team (Askmet has this feature though)
